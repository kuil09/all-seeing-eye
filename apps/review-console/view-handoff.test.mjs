...
test("buildViewHandoffSummary carries the next pending event when the selection is already reviewed", () => {
  ...
  assert.equal(
    summary.helperCopy,
    "Copy this reviewed context, or hand off the next pending event below."
  );
  assert.deepEqual(summary.selectedContextItems, [
    "Confidence: medium confidence 61%",
    "Provenance: 1 source across 1 feed",
...
test("buildViewHandoffSummary keeps portability note when demo mode changes the replayed state", () => {
  ...
  assert.equal(
    summary.portabilityNote,
    "Empty demo stays in the URL so the next viewer reopens this QA state instead of the normal queue."
  );
});

test("buildViewHandoffSummary combines browser-local and demo-mode caveats in one portability note", () => {
  const summary = buildViewHandoffSummary({
    selectedHeadline: "Storm-related outage affects East Grid substation 7",
    filteredCount: 1,
    totalCount: 7,
  ...
  assert.equal(
    summary.portabilityNote,
    'The copied start link reopens this queue, but draft note text and saved view label "Ports needing edits" stay in this browser only. Empty demo stays in the URL so the next viewer reopens this QA state instead of the normal queue. Copy review note includes the current draft snapshot for reviewer context.'
  );
});

test("buildViewHandoffSummary keeps demo-mode warning when saved-draft replay is already flagged", () => {
  const summary = buildViewHandoffSummary({
    selectedHeadline: "Storm-related outage affects East Grid substation 7",
    filteredCount: 1,
    totalCount: 7,
    sourceLabel: "Local read API",
    filterSummary: {
      activeFilters: ["Drafts: saved"],
      hasActiveFilters: true,
      demoModeLabel: "Error demo",
      sortLabel: null
    },
    draftFilter: "saved",
    demoMode: "error"
  ...
  assert.equal(
    summary.portabilityNote,
    "Saved-draft filtering stays in the copied start link, but it only reproduces on browsers that already have matching local drafts. Use Copy start link without saved drafts to remove this dependency. Error demo stays in the URL so the next viewer reopens this QA state instead of the normal queue."
  );
});
...
test("buildViewHandoffPreviewItems groups the extended handoff context for the card preview", () => {
  ...
  assert.deepEqual(buildViewHandoffPreviewItems(handoffSummary), [
    ...
    },
    {
      ...
      value: "Portable handoff should keep this event selected."
    },
    {
      label: "Source proof",
      value:
...
test("buildViewHandoffPreviewSummary matches the detail types present in the preview", () => {
  const reviewerContextOnly = buildViewHandoffSummary({
    selectedHeadline: "Inspection surge reported at Harbor North cargo terminal",
    filteredCount: 2,
    totalCount: 7,
    sourceLabel: "Contract fixtures",
  ...
  const evidenceOnly = buildViewHandoffSummary({
    selectedHeadline: "Inspection surge reported at Harbor North cargo terminal",
    filteredCount: 2,
    totalCount: 7,
    sourceLabel: "Contract fixtures",
  ...
  const mixedPreview = buildViewHandoffSummary({
    selectedHeadline: "Inspection surge reported at Harbor North cargo terminal",
    filteredCount: 2,
    totalCount: 7,
    sourceLabel: "Contract fixtures",
  ...
  assert.equal(
    buildViewHandoffPreviewSummary(reviewerContextOnly).label,
    "Show reviewer context"
  );
  assert.equal(
    buildViewHandoffPreviewSummary(reviewerContextOnly).countLabel,
    "1 context item"
  );
  assert.equal(buildViewHandoffPreviewSummary(evidenceOnly).label, "Show evidence");
  assert.equal(buildViewHandoffPreviewSummary(evidenceOnly).countLabel, "1 evidence item");
  assert.equal(
    buildViewHandoffPreviewSummary(mixedPreview).label,
    "Show reviewer context and evidence"
  );
  assert.equal(
    buildViewHandoffPreviewSummary(mixedPreview).countLabel,
    "1 context item · 1 evidence item"
  );
});
...
test("buildViewHandoffNote includes portable and local-only scope details when needed", () => {
  ...
  assert.equal(
    ...
    [
      ...
      '- Portability note: Saved-draft filtering stays in the copied start link, but it only reproduces on browsers that already have matching local drafts. Use Copy start link without saved drafts to remove this dependency. The copied start link reopens this queue, but draft note text and saved view label "Ports needing edits" stay in this browser only. Copy review note includes the current draft snapshot for reviewer context.'
    ].join("\n")
  );
});
