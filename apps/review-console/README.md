...
- Success flash notes now offer a one-click `Reopen reviewed event for context` path for the event that was just reviewed and show the queue lens that button will restore, so analysts can immediately verify or revise the last decision without losing the next-pending queue handoff
