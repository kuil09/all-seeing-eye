...
- Shareable view now also carries the selected event's active search-match rationale plus current queue position and next-pending context into the handoff card and copied note, exposes a direct `Copy next pending link` action when the current selection is context only, includes a direct next-pending link in copied notes when another actionable item is waiting, and copied links now restore the focused detail section when one is active, so async reviewers can reopen the exact matched subsection instead of rescanning detail
- Shareable view now keeps the recommended reopen path in copied notes and on live cards that do not expose a direct next-pending button, while context-only live cards keep a shorter helper line once the direct next-pending action and named next-pending headline are already visible
