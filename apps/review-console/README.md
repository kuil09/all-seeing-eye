...
- Copy handoff note now emits action-first markdown sections with labeled current, portable, and next-pending links, keeps reviewer context focused on triage cues, and moves raw supporting source proof into an evidence appendix so pasted async handoffs stay scannable even when proof-rich detail is preserved
