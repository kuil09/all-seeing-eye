...
- Shareable view now also offers a portable-link copy path that strips saved-draft filtering so async handoff does not depend on browser-local state, and only surfaces portability warning copy when saved-draft, demo-mode, or other browser-local caveats actually matter
...
- Shareable view now folds the long-form handoff evidence into an expandable preview inside the card while keeping the copied note fully detailed, so the control panel stays scannable during live triage without dropping async reviewer context
...
- Shareable view card controls now use reviewer-centered wording such as `Copy start link`, `Copy start link without saved drafts`, `Copy review note`, and `Show reviewer context and evidence`, so the live handoff surface speaks in terms of the next review action instead of implementation jargon
- Copy review note now emits action-first markdown sections with `Start here` and `Continue with next pending` reopen labels instead of current/portable-view jargon, trims repeated queue/view phrasing in the queue snapshot and recommended-path copy, keeps reviewer context focused on triage cues, moves raw supporting source proof into an evidence appendix, and avoids repeating reviewer-context/evidence section labels again inside `Handoff scope` so pasted async handoffs stay scannable even when proof-rich detail is preserved
...
- Recent activity keeps the last local review decisions visible in the controls panel, previews the latest analyst note, uses chips for reopen queue position, surfaces the saved search/filter/sort lens that reopening will restore, keeps the helper copy focused on pending versus context recovery, names the next pending headline when the card is now context only, and restores that note into the draft editor when the event is reopened, so auto-advance does not strand the last decision or its rationale
- Success flash notes now offer a one-click `Reopen reviewed event for context` path for the event that was just reviewed, show the queue lens that button will restore, and preview the last analyst note when one will be reloaded into the draft editor, so analysts can immediately verify or revise the last decision without losing the next-pending queue handoff
...
- Recent activity remains local to the browser and captures the last reviewed event, the latest analyst note, and the saved reopen filters plus queue sort needed to inspect it again quickly, while the live card resolves the current reopen queue position from that saved view state
