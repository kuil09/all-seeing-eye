...
- Shareable view now keeps the recommended reopen path on live cards that do not expose a direct next-pending button, while copied notes drop the extra `Recommended path` line once action-first reopen links already spell out both the context and next-pending flow
