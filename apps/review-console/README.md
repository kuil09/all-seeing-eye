...
- Shareable view now also surfaces a reviewer snapshot plus recent persisted review context for the selected event, trims redundant `pending review` status, drops empty `No prior review` copy, and keeps reviewed-status chips behind confidence and provenance cues so the snapshot stays scan-friendly
...
- Copy handoff note now emits action-first markdown sections with labeled current, portable, and next-pending links, keeps reviewer context focused on triage cues, moves raw supporting source proof into an evidence appendix, and summarizes note-only scope at the section level so pasted async handoffs stay scannable even when proof-rich detail is preserved
