...
- Shareable view now also carries the selected event's active search-match rationale plus current queue position and next-pending context into the handoff card and copied note, includes a direct next-pending link when another actionable item is waiting, and copied links now restore the focused detail section when one is active, so async reviewers can reopen the exact matched subsection instead of rescanning detail
...
- Copy review note now emits action-first markdown sections with `Start here` and `Continue with next pending` reopen labels instead of current/portable-view jargon, trims repeated queue/view phrasing in the queue snapshot and recommended-path copy, keeps reviewer context focused on triage cues, moves raw supporting source proof into an evidence appendix, and summarizes note-only scope at the section level so pasted async handoffs stay scannable even when proof-rich detail is preserved
