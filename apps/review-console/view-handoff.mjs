export function buildViewHandoffSummary({
  ...
  return {
    ...
    helperCopy: showNextPendingCopyAction
      ...
      : cleanedSelectedHeadline
        ? "Copy a start link for this queue slice and selected event."
        : "Copy a start link for this queue slice.",
    includedState,
    localDependentState,
    localOnlyState,
    noteOnlyState,
    ...
    portabilityNote: buildPortabilityNote({
      draftFilter,
      demoMode,
      demoModeLabel: normalizeLabel(filterSummary.demoModeLabel),
      hasSelectedDraft,
...
export function buildViewHandoffPreviewItems(handoffSummary = {}) {
  ...
  if (normalizeLabel(handoffSummary.selectedDraftPreview)) {
    previewItems.push({
      ...
      value: handoffSummary.selectedDraftPreview
    });
  }
...
export function buildViewHandoffPreviewSummary(handoffSummary = {}) {
  const reviewerContextCount = countViewHandoffReviewerContextItems(handoffSummary);
  const evidenceCount = countViewHandoffEvidenceItems(handoffSummary);
  const hasReviewerContext = reviewerContextCount > 0;
  const hasEvidence = evidenceCount > 0;
  const countSegments = [
    reviewerContextCount
      ? formatPreviewCountLabel(reviewerContextCount, "context item")
      : "",
    evidenceCount ? formatPreviewCountLabel(evidenceCount, "evidence item") : ""
  ].filter(Boolean);

  return {
    label:
      hasReviewerContext && hasEvidence
...
function countViewHandoffReviewerContextItems(handoffSummary = {}) {
  return [
    normalizeLabel(handoffSummary.selectedConfidenceContext),
    normalizeLabel(handoffSummary.selectedReviewContext),
    normalizeLabel(handoffSummary.selectedSearchContext),
    normalizeLabel(handoffSummary.selectedDraftPreview)
  ].filter(Boolean).length;
}

function countViewHandoffEvidenceItems(handoffSummary = {}) {
  const sourceProofCount = Array.isArray(handoffSummary.selectedSourceProofItems)
    ? handoffSummary.selectedSourceProofItems.filter((item) => normalizeLabel(item)).length
    : 0;

  return sourceProofCount + (normalizeLabel(handoffSummary.selectedSourceProofOverflowCopy) ? 1 : 0);
}

function formatPreviewCountLabel(count, singularLabel) {
...
function buildPortabilityNote({
  ...
  const portabilityNotes = [];

  if (draftFilter === "saved") {
    portabilityNotes.push(
  ...
  if (demoMode !== "normal") {
    portabilityNotes.push(
      `${normalizeLabel(demoModeLabel) || "Demo mode"} stays in the URL so the next viewer reopens this QA state instead of the normal queue.`
    );
  }

  if (!portabilityNotes.length) {
    return "";
  }

  return appendDraftSnapshotNote(portabilityNotes.join(" "), hasSelectedDraftSnapshot);
}
