...
test.describe("shareable view handoff", () => {
  ...
  test("copy handoff note includes current and portable links plus scope cues", async ({
    ...
    await expect(handoffPreview).toContainText("4 context items · 2 evidence items");
  ...
  test("copy next pending link strips saved-draft filtering when the direct handoff would stay browser-local", async ({
    page
  }) => {
    await page.addInitScript(() => {
      window.__copiedText = "";
    ...
    const harborCard = page
      ...
      .filter({ hasText: reviewedHeadline })
      .first();
    const eastGridCard = page
      .locator("#timeline-list .timeline-card")
      .filter({ hasText: nextPendingHeadline })
      .first();

    await expect(harborCard).toBeVisible();
    await expect(eastGridCard).toBeVisible();

    await eastGridCard.click();
    await page.locator("#review-notes").fill("Pending handoff should stay portable.");
    ...
    await harborCard.click();
    await expect(page.locator(".detail-shell h2").first()).toHaveText(reviewedHeadline);
    await page.locator("#review-notes").fill("Reviewed context stays in this browser.");
    ...
    await page.getByRole("button", { name: /Saved drafts/i }).click();
    const copyNextPendingButton = page.getByRole("button", {
      name: "Copy next pending link without saved drafts"
    });
    await expect(copyNextPendingButton).toBeVisible();

    await copyNextPendingButton.click();
    await expect(page.locator(".view-handoff-note.is-success")).toContainText(
